...
# FIXME: make arguments not positional
...
def main():
    ...
    if not os.path.exists(directory_to_process):
        ...
        sys.exit(1)  # Exit early with an error code

    # Check which arguments are provided and calls the correct method
    if args.date and args.time:
        prepend_date_time_to_files(directory_to_process, args.date, args.time)
    elif args.date:
        prepend_date_time_to_files(directory_to_process, args.date)
    elif args.time:
